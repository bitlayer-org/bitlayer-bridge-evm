...
library BridgeUtils {
    ...
    uint32 public constant TRANSFER_STAKE_REQUIRED = 6666;
    ...
    // token Ids, BTC is not supported yet
    uint8 public constant BTC = 99;

    string public constant MESSAGE_PREFIX = "DST_BRIDGE_MESSAGE";
    ...
    /// @notice Converts the provided currrent chain amount to the other chain's decimal adjusted amount.
    /// @param erc20Decimal token decimal of current chain.
    /// @param dstDecimal token decimal of other chains.
    /// @param amount token amount of current chain.
    /// @return token amount of other chains.
    function convertERC20ToDstDecimal(uint8 erc20Decimal, uint8 dstDecimal, uint256 amount)
        internal
        pure
        returns (uint64)
    {
        if (erc20Decimal == dstDecimal) {
        ...
        if (erc20Decimal > dstDecimal) {
            amount = amount / (10 ** (erc20Decimal - dstDecimal));
        } else {
            amount = amount * (10 ** (dstDecimal - erc20Decimal));
        }
    ...
    /// @notice Converts the provided other chains's decimal adjusted amount to the current chain token amount.
    /// @param erc20Decimal token decimal of current chain.
    /// @param dstDecimal token decimal of other chains.
    /// @param amount token amount of other chains.
    /// @return token amount of current chain.
    function convertDstToERC20Decimal(uint8 erc20Decimal, uint8 dstDecimal, uint64 amount)
        internal
        pure
        returns (uint256)
    {
        if (dstDecimal == erc20Decimal) {
            return uint256(amount);
        }

        uint256 _amount = uint256(amount);
        if (erc20Decimal > dstDecimal) {
            _amount = amount * (10 ** (erc20Decimal - dstDecimal));
        } else {
            _amount = amount / (10 ** (dstDecimal - erc20Decimal));
        }
        return _amount;
    }
        ...
        returns (TokenTransferPayload memory)
    {
        ...
        require(
            ...
            "BridgeUtils: Invalid sender address length, destination chain address must be 32 bytes"
        );
    ...
    /// @return newLimit the new limit of the sending chain ID and token ID.
    function decodeUpdateLimitPayload(bytes memory _payload)
        internal
        pure
        returns (uint8 senderChainID, uint8 senderTokenID, uint256 newLimit)
    {
        require(_payload.length == 18, "BridgeUtils: Invalid payload length");
    ...
    ///     byte m + 1       : number of destination chain token decimals
    ///     bytes m + 2 -> i : destination chain token decimals
    ...
    /// @return decimals the destination chain decimal places of the tokens to be added.
        ...
        )
    {
        ...
        decimals = new uint8[](decimalCount);
        for (uint8 i; i < decimalCount; i++) {
            decimals[i] = uint8(_payload[offset++]);
        }
