...
interface IBridgeCommittee {
    ...
    function submitterlist(address member) external view returns (bool);
    ...
    /// @notice Emitted when the submiter state is updated.
    /// @param member The addresses of the updated committee members.
    /// @param state A boolean indicating whether the committee members are submitter state true or false.
    event SubmitterStateUpdated(address member, bool state);
}
