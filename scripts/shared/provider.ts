export interface EthereumProvider {
  ...
  send(
    ...
    params: [string]
  ): Promise<null | EthereumTransaction>
  send(
    method: 'eth_getTransactionReceipt',
    params: [string]
  ): Promise<null | EthereumTransactionReceipt>
 ...
 */
export async function getHardhatMetadata(
  provider: EthereumProvider
): Promise<HardhatMetadata> {
 ...
 */
export async function getAnvilMetadata(
  provider: EthereumProvider
): Promise<HardhatMetadata> {
...
export async function getTransactionReceipt(
  ...
  if (receipt?.status) {
    receipt.status = receipt.status.match(/^0x0+$/)
      ? '0x0'
      : receipt.status.replace(/^0x0+/, '0x')
  }
  return receipt
}
